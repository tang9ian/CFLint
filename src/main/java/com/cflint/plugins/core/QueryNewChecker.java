    ...
    public void expression(final CFExpression expression, final Context context, final BugList bugs) {
        if (expression instanceof CFFunctionExpression) {
            ...
            if (functionExpression.getName().equalsIgnoreCase("querynew") && functionExpression.getArgs().size() > 0) {
                ...
                // Build the list, then get unique with a set, pull them out of the list,
                // anything
                // left is a duplicate.
                final List<String> cols = new ArrayList<>(Arrays.asList(query.toLowerCase().split("\\s*,\\s*")));
                final Set<String> cols_unique = new HashSet<>(cols);
