...
public class TooManyFunctionsChecker extends CFLintScannerAdapter {
    ...
    protected void checkNumberFunctions(final int functionCount, final int atLine, final int atOffset,
            final Context context, final BugList bugs, final CFExpression cfExpression) {
        ...
        if (functionCount == threshold + 1) {
            context.getParent(ContextType.COMPONENT).addUniqueMessage("EXCESSIVE_FUNCTIONS", null, this, atLine,
                    atOffset, cfExpression);
        }
    }
